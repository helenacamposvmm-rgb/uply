...
const fragmentShader = `
  ...
  void main() {
    ...
    gl_FragColor = cppn_fn(st, 0.1 * sin(0.3 * iTime), 0.1 * sin(0.69 * iTime), 0.1 * sin(0.44 * iTime));
  }
`;
...
function ShaderBackground() {
  ...
  const camera = useMemo(() => ({ position: [0, 0, 2], fov: 75, near: 0.1, far: 1000 }), []);
  ...
  return (
    <div ref={sectionRef} className="relative h-full w-full overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col items-center justify-center gap-6 px-6 text-center sm:gap-8 md:px-10 lg:px-16">
        ...
        <h1 ref={headerRef} className="max-w-4xl text-5xl font-light leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl">
          {title}
        </h1>

        <p ref={paraRef} className="max-w-xl text-base font-light leading-relaxed tracking-tight text-white/75 sm:text-lg">
          {description}
        </p>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center justify-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-full border border-white/10 px-6 py-3 text-sm font-light tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-white/30 duration-300 ${
                button.primary
